...
public class TomsHardwareNewsFinder extends NewsFinder {
    public TomsHardwareNewsFinder() {
        super();
        this.baseURl="https://www.tomshardware.com/reviews";
        this.publicationName="Tom's Hardware";
        this.logoID= R.drawable.toms_hardware_logo;
    ...
    public List<String> findArticles(String url) {
        ...
        Matcher m= Pattern.compile("<a href=\".+?\" class=\"article-link\" aria-label=\".+?\">").matcher(url);
        while(m.find())
        {
    ...
    public String retrieveTitle(String article) {
        ...
        Matcher m=Pattern.compile("aria-label=\"(.+?)\"").matcher(article);
        if(m.find())
        {
            title=m.group(1);
        }
